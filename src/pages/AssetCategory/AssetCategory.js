...
export const AssetCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await assetCategoryService.createAssetCategory(
        assetCategory
      );
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const deleteAssetCtr = async () => {
          try {
            await assetCategoryService.deleteAssetCategory(name);
            onGetAllAssetCategory();
            let event = {
  ...
  const onFilter = async () => {
    if (dropdownName === "Subproduct") {
      setLoading(true);
      try {
        const response = await assetCategoryService.getDataBySubproductLike(
          filter
        );
        setData(response.data);
      } catch (e) {
      ...
      setLoading(true);
      try {
        const response = await assetCategoryService.getDataByProductLike(
          filter
        );
        setData(response.data);
      } catch (e) {
      ...
      setLoading(true);
      try {
        const response = await assetCategoryService.getDataByAssetCategoryLike(
          filter
        );
        setData(response.data);
      } catch (e) {
  ...
  const handleEdit = async (e, id) => {
    ...
    try {
      assetCategory.useful_life = Number(assetCategory.useful_life);
      const response = await assetCategoryService.updateAssetCategory(
        id,
        assetCategory
      );
          ...
          <div className="container">
                ...
                <ul className="dropdown-menu">
                  ...
                  <li>
                    <a
                      className="dropdown-item"
                      onClick={() => {
                        onChangeDropdown("Asset Category");
                      }}
                    >
                      Asset Category
                    </a>
                    ...
                    currentItems.map((item, index) => (
                      <tr key={item.subproduct_name}>
                        ...
                        <td>
                          ...
                          <a
                            onClick={() =>
                              onDeleteAssetCategory(item.subproduct_name)
                            }
                            className="delete"
                            data-toggle="modal"
                            style={{ cursor: "pointer" }}
                  ...
                  <form onSubmit={handleSubmit}>
                    ...
                    <div>
                      ...
                      <div className="form-group mt-3">
                        <label className="form-label">
                          Useful Life<span style={{ color: "red" }}>*</span>
                        </label>
                        <input
                          style={{ maxWidth: "500px" }}
                          required
                      ...
                      <div className="form-group mt-3">
                        <label className="form-label">
                          Subproduct Name<span style={{ color: "red" }}>*</span>
                        </label>
                        <input
                          style={{ maxWidth: "500px" }}
                          required
            ...
            <div className="model-box-view">
              ...
              >
                ...
                <Modal.Body>
                  <form
                    onSubmit={(e) => handleEdit(e, RowData.subproduct_name)}
                  >
                    <div>
                      <div className="form-group">
